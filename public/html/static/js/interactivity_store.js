...
$.post(apisUrl + '/get_store_info', function(data, textStatus, jqXHR){
    ...
    for (let i=0;i<data.length;i++){
        booth2store[data[i].store_berth]=data[i].store_name
    }
    console.log('all strores', booth2store);
});
...
$('#datetime-body').on('apply.daterangepicker',function(){
    ...
    if (store_id_sel){
        loadRangeData(startDateTime,endDateTime,booth2store[store_id_sel]);
    }
...
floorGroup.on('loaded',function(){
    ...
    if(Object.keys(booth2store).includes(hit.mesh.id)){
...
function loadRangeData(startDateTime,endDateTime,store_id){
    ...
    // $.post(apisUrl + '/get_matching', {'store_id': store_id}, function(data, textStatus, jqXHR){
    //     if (textStatus=='success'){
    //         console.log(data)
    //     }
    //     else alert('fail loading data');
...
function createCircBarChart(data){
    ...
    var options = {
        series: [enter_tot/passer_tot*270, exit_tot/passer_tot*270, watcher_tot/passer_tot*270, (passer_tot-enter_tot-exit_tot-watcher_tot)/passer_tot*270],
        chart: {
        height: 200,
        type: 'radialBar',
      },
